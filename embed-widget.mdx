...
Render custom visuals inline in the chat when a tool completes (charts, previews, cards).

<Tabs>
  <Tab title="Script Tag">
    Script-tag renderers return an **HTML string**. Use inline `style` attributes (Tailwind classes won't work inside the widget's Shadow DOM):
    ...
    window.crow('registerToolRenderers', {
      query_analytics_chart: ({ result }) => {
        ...
        return `<div style="padding:12px; border:1px solid #e2e8f0; border-radius:8px; background:#fff; font-family:system-ui,sans-serif;">
          <div style="font-size:13px; font-weight:600; color:#0f172a;">${r.title || 'Chart'}</div>
    ...
    **Timing:** `window.crow()` must be called **after** `crow-widget.js` has loaded. In plain HTML this happens naturally (scripts execute in order). In Next.js, use `onLoad`:
    ...
    <Tip>
    The widget uses Shadow DOM for CSS isolation. Use inline `style` attributes in your HTML strings — Tailwind classes and external CSS won't apply inside the widget.
    </Tip>
  </Tab>
